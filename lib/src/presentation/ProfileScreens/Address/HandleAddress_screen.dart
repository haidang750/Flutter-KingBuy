...
class HandleAddress extends StatefulWidget {
  HandleAddress({this.address, this.event});
  Address address;
  String event;
  @override
  HandleAddressState createState() => HandleAddressState();
}

class HandleAddressState extends State<HandleAddress> {
  ...
  Future<void> checkDistrict() async {
    districtDropdownValue = await addressViewModel
        .getDistrict(provinceDropdownValue)
        .then((districts) => districts
                .any((district) => district.code == widget.address.districtCode)
            ? widget.address.districtCode
            : "");
  }

  Future<void> checkWard() async {
    wardDropdownValue = await addressViewModel
        .getWard(widget.address.districtCode)
        .then((wards) =>
            wards.any((ward) => ward.code == widget.address.wardCode)
                ? widget.address.wardCode
                : "");
  }
  ...
  Widget buildProvinceField(){
    return Container(
        ...
        child: StreamBuilder(
          stream: addressViewModel.provinceStream,
  ...
  Widget buildDistrictField(){
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.all(Radius.circular(8))),
        child: StreamBuilder(
          stream: addressViewModel.districtStream,
          builder: (context, snapshot) {
            return DropdownButton<String>(
                isExpanded: true,
                underline: Container(),
                ...
                items: snapshot.hasData
                    ? snapshot.data.map<DropdownMenuItem<String>>(
                      ...
                      return DropdownMenuItem<String>(
                        value: district.code,
                        child: Text(district.nameWithType == null
                            ? ""
  ...
  Widget buildWardField(){
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.all(Radius.circular(8))),
        child: StreamBuilder(
          stream: addressViewModel.wardStream,
          builder: (context, snapshot) {
            return DropdownButton<String>(
                isExpanded: true,
                underline: Container(),
                ...
                items: snapshot.hasData
                    ? snapshot.data
                    .map<DropdownMenuItem<String>>((Ward ward) {
                  return DropdownMenuItem<String>(
                    value: ward.code,
                    child: Text(ward.nameWithType == null
                        ? ""
                        : ward.nameWithType),
                  );
                }).toList()
      ...
      child: Row(
        ...
        children: [
          ...
          buildButton(
              ...
              60,
              100,
              isDefault == 1 ? Colors.grey : Colors.blue,
              30,
              "Áp dụng",
              Colors.white,
              16,
              FontWeight.w500,
              handleButtonApplyDefault),
          SizedBox(
            width: 10,
          ),
          buildButton(
  ...
  void handleButtonCreate() async {
    bool result = await addressViewModel.createDeliveryAddress(
        context,
  ...
  void handleButtonUpdate() async {
    bool result = await addressViewModel.updateDeliveryAddress(
        context,
        deliveryAddressId,
        fullNameController.text,
        firstPhoneController.text,
  ...
  void handleButtonDelete() async {
    bool result = await addressViewModel.deleteDeliveryAddress(
      context,
      deliveryAddressId,
    );
    if (result) {
