...
import 'package:projectui/src/presentation/CategoriesScreens/Category.dart';
import 'package:projectui/src/presentation/CategoriesScreens/CategoryDetail_screen.dart';
...
class _RootCategoriesScreenState extends State<RootCategoriesScreen> {
        ...
        body: StreamBuilder(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              return Row(
                children: [
                  listCategoriesContainer(snapshot.data.categories),
                  oneCategoryContainer(snapshot.data.categories)
                ],
              );
            } else {
              return Center(
                ...
                child: Column(
                  children: [
                    categoryName(
                        ...
                        () => index == 0
                            ? null
                            : handleTouchCategory(categories[index - 1])),
                    categoryBody(
                        index,
                        index >= 1 ? categories[index - 1].backgroundImage : "",
                        index >= 1 ? categories[index - 1].children : [])
                  ],
                ),
              )),
      onPageChanged: (value) {
        ...
        scrollController.animateTo(
            currentIndex * MediaQuery.of(context).size.width * 0.3,
  ...
  Widget buildListCategories(
      int index, List<Category> categories, Function action) {
    return Container(
          ...
          Expanded(
            ...
            child: Container(
                ...
                child: index == 0
                    ...
                    : buildOneCategory(categories[index - 1].name,
                        categories[index - 1].imageSource)),
            onTap: action,
          ))
        ],
      ),
  ...
  Widget categoryBody(
      int index, String backgroundImage, List<Category> childCategories) {
    if (index == 0) {
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            return Container(
                ...
                child: buildListChildCategories(snapshot.data.categories));
          } else {
          ...
          children: [
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                    image: DecorationImage(
                        image: NetworkImage(
                            "${AppEndpoint.BASE_URL}$backgroundImage"),
                        fit: BoxFit.fill),
            ...
            Expanded(
              child: Container(
                  ...
                  child: buildListChildCategories(childCategories)),
            )
          ],
        ),
      );
